...
first real run ([32182623079](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32182623079))
executed every step in `.github/workflows/ci.yml` on a real Ubuntu runner and passed;
its second real run ([32221241397](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32221241397))
passed against a raised `MIN_TESTS` floor (397) and confirmed the suite still collects
and passes at that count; its third real run ([32288583970](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32288583970))
passed against a further-raised `MIN_TESTS` floor (425), closing the local-vs-CI
divergence the earlier two runs still had; its fourth real run
([32295996939](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32295996939))
passed against a further-raised `MIN_TESTS` floor (429, the never-publish export-ignore
boundary work folded in) — 429 collected, 428 passed, 1 skipped, contracts 1 kept / 0
broken; its fifth real run
([32631982636](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32631982636))
passed against the final `MIN_TESTS` floor (466, the analyze/`declare-task-class`/archive-
boundary work folded in) — 466 collected, 465 passed, 1 skipped, contracts 1 kept / 0
broken; its sixth real run
([34024918565](https://github.com/mrlngaur-glitch/shipgate/actions/runs/34024918565))
passed against the same `MIN_TESTS` floor (466) after the public repo was re-synced from
the private source-of-truth (`mrlngaur-glitch/shipgate_private`) — 466 collected, 465
passed, 1 skipped, contracts 1 kept / 0 broken; its seventh real run
([34025172117](https://github.com/mrlngaur-glitch/shipgate/actions/runs/34025172117))
passed after the doc-sync update folded the previous run's ID into `README.md`,
`SECURITY.md`, and `ci.yml` — 466 collected, 465 passed, 1 skipped, contracts 1 kept / 0
broken. Three earlier pushes had failed to start at all (zero jobs, no logs) — the cause
was an account-level billing lock on the GitHub account this repository is hosted under,
unrelated to this file, `ci.yml`, or any code in this repository; it has since been resolved,
and is why this note existed in the first place. Every "CI-enforced" / "on every build"
claim below is now re-confirmed against those seven real runs, not left as the
disk-verified-only status this note used to require. (This project's own commits since run
34025172117 may carry doc-accuracy updates that are one run behind live CI, as the
doc-sync mechanism requires a commit to exist before CI can run against it;
`LATEST_CI_RUN_ID` in `ci.yml` names the run this section describes, and is updated by hand
...
- **Secrets never reach the ledger** — the full `tests/unit/test_redaction.py` and
  ...
  passed on all seven runs; they are ordinary members of the 429 collected at run
  32295996939, not a separate CI step.
- **The ledger is append-only, structurally** — `tests/unit/test_ledger.py`'s
  ...
  table plus an earlier-row-tampered case) ran in CI and passed on all seven runs, same
  basis as above.
- **Dependencies are pinned, hashed, and audited** — the dev/CI set
  ...
  succeeded on all seven runs (closing the "manylinux x86_64... will be tested the first
  ...
  (`requirements-audit.lock`), and reported no known vulnerabilities on all seven runs —
  ...
  are exercised by definition on every CI run, including these seven.
- **Core purity is enforced by a real check** — `import-linter`'s "Core purity contract"
  step ran in CI and passed (1 kept, 0 broken) on all seven runs, not only locally.
...
- **The dev/CI environment** — what this project's own tests, lint, and `pip-audit` run
  ...
  **Hash-covered and tested-on are two different claims, kept separate** — an earlier
  ...
  first five real runs ([32182623079](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32182623079),
  [32221241397](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32221241397),
  [32288583970](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32288583970),
  [32295996939](https://github.com/mrlngaur-glitch/shipgate/actions/runs/32295996939))
...
- **The GitHub Actions that execute CI itself** — `actions/checkout` and
  ...
  version kept as a trailing comment. Bumped once already, 2026-08-19, when GitHub began
  emitting Node 20 deprecation warnings for the original pins (`v4.4.0`, `v5.6.0`) — each
  new SHA re-verified the same way (resolved against the tag API, the commit confirmed to
  exist on the official repo, `action.yml` at that SHA confirmed to declare `node24`), one
  action at a time. Current pins: `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1  # v7.0.1`
  and `actions/setup-python@5fda3b95a4ea91299a34e894583c3862153e4b97  # v7.0.0`. A pinned SHA
...
first five real runs all executed this step and all reported **no known
vulnerabilities** (see the run-by-run list above). A manual `pip-audit -r requirements.lock` pass on **2026-08-17**,
...
- **`sessions` and `supersessions` are append-only (trigger-enforced) but not
  hash-chained** — a deliberate, reasoned scope decision (`shipgate/ledger/hashing.py`'s
  own module docstring), not an oversight: `sessions` rows are ingest bookkeeping, and
  `supersessions` is itself the correction mechanism for the other tables, neither one
  is what a Ship Report's self-verifying receipt depends on. `shipgate report --verify`
  re-derives exactly the three tables that are hash-chained (`events`, `claims`,
  `verdicts`) and says so precisely — named here explicitly so a reader doesn't have to
  infer the boundary from what isn't mentioned.
