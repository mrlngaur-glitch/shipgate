...
def _pytest_invocation(pytest_args: list[str] | None, base_argv: list[str] | None = None) -> list[str]:
    """`base_argv` is the resolved interpreter/executable prefix to actually run —
    e.g. `["C:\\proj\\venv\\Scripts\\python.exe", "-m", "pytest"]` or
    `["C:\\proj\\venv\\Scripts\\pytest.exe"]`. Defaults to `[sys.executable, "-m",
    "pytest"]`, this process's own interpreter, which is only correct when the caller
    (a shipfile `tests_pass` command) named no specific interpreter at all — a bare
    `pytest`/`python`/`python3`. **Seventh founder finding (`shipgate.gate.checkers`'
    module docstring): silently substituting `sys.executable` for an EXPLICITLY named
    interpreter path was itself a false-green defect** — `check_tests_pass` is
    responsible for resolving a named path and passing it through here as `base_argv`
    so the command that actually runs is the one the shipfile actually named, not
    whichever Python happens to be running the gate."""
    return [*(base_argv or [sys.executable, "-m", "pytest"]), *_strip_verbosity_flags(pytest_args)]


def collect_test_count(
    cwd: Path,
    pytest_args: list[str] | None = None,
    timeout: float = DEFAULT_PYTEST_TIMEOUT_SECONDS,
    ...
    exactly as much "no evidence" as the real run hanging. `base_argv`: see
    `_pytest_invocation`. Raises `FileNotFoundError` (uncaught, an `OSError` subclass)
    if `base_argv`'s own executable does not exist — the caller (`check_tests_pass`)
    is expected to resolve and confirm a named interpreter exists *before* calling
    this, so that case renders a clean `UNVERIFIED` instead of an uncaught exception;
    this function itself makes no verdict decision, same as its timeout behavior."""
    args = [*_pytest_invocation(pytest_args, base_argv), "--collect-only", "-q"]
    proc = subprocess.run(
...
def run_pytest(
    ...
    phase gets its own full budget, not a shared one split between them. `base_argv`:
    see `_pytest_invocation` — threaded through to both phases identically, so
    `PytestResult.command` (the real run's argv) is always the actual base_argv used,
    never silently `sys.executable` when something else was named. Raises
    `subprocess.TimeoutExpired` (uncaught) if either phase exceeds it, and
    `FileNotFoundError` (uncaught) if `base_argv`'s executable doesn't exist; see the
    module docstring for why this module doesn't turn either into a verdict itself."""
    collected = collect_test_count(cwd, pytest_args, timeout=timeout, base_argv=base_argv)

    with tempfile.TemporaryDirectory() as tmp_dir:
        junit_path = Path(tmp_dir) / "junit.xml"
        args = [*_pytest_invocation(pytest_args, base_argv), "-q", f"--junitxml={junit_path}"]
        proc = subprocess.run(
            args, cwd=cwd, capture_output=True, text=True, encoding="utf-8", errors="replace",
            check=False, timeout=timeout,
        )
