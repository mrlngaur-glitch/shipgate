...
scope addition, outside the original Launch Month plan. Every test here is runtime-verified
...
# --- founder-found defect: a synthetic fixture aggregated as a real project ---------


def test_multiple_sessions_sharing_one_identical_timestamp_is_flagged_synthetic(tmp_path):
    """Reproduces, at small scale, exactly what a live 2000-session perf fixture
    (`scratch_perf2/`, untracked, not part of the shipped repo) did when pointed at by
    `--roots`: every session sharing one identical first/last-activity timestamp
    rendered as an ordinary, unflagged '100% green' project. `_seed_healthy_ledger`'s
    default `event_timestamp` is the same constant across calls unless overridden, so
    two sessions seeded without changing it reproduces the exact same shape."""
    db_path = _make_project(tmp_path, "proj-synthetic")
    _seed_healthy_ledger(db_path, session_id="s1")
    _seed_healthy_ledger(db_path, session_id="s2")  # same default event_timestamp

    result = analyze([tmp_path])
    assert len(result.projects) == 1
    project = result.projects[0]
    assert project.session_count == 2
    assert project.first_activity == project.last_activity
    assert project.looks_synthetic is True


def test_a_single_session_is_never_flagged_synthetic_even_with_one_timestamp(tmp_path):
    """A brand-new project with exactly one session naturally has an identical
    first/last activity — that is real, ordinary data, not a fixture's signature. Only
    *multiple* sessions collapsing onto one instant is the flagged shape."""
    db_path = _make_project(tmp_path, "proj-new")
    _seed_healthy_ledger(db_path, session_id="s1")
    ...
    assert project.first_activity == project.last_activity
    assert project.looks_synthetic is False


def test_synthetic_looking_project_shown_but_excluded_from_cross_project_totals(tmp_path):
    """Never a silent drop (the inventory rule, applied to this command's own output):
    the synthetic-looking project is still rendered in full in its own section, with a
    loud warning, but is named and excluded from the cross-project aggregate so it
    cannot skew the single statistic most likely to be quoted."""
    synthetic_path = _make_project(tmp_path, "proj-synthetic")
    _seed_healthy_ledger(synthetic_path, session_id="s1")
    _seed_healthy_ledger(synthetic_path, session_id="s2")

    real_path = _make_project(tmp_path, "proj-real")
    _seed_healthy_ledger(real_path, session_id="s3", event_timestamp="2026-08-16T00:01:00Z")
    ...
    assert "⚠ SYNTHETIC-LOOKING DATA" in text
    assert "proj-synthetic" in text  # named in the exclusion line, not silently dropped
    assert "1 project(s) excluded from the totals below as synthetic-looking data" in text

    real_projects = [p for p in result.projects if not p.looks_synthetic]
    assert len(real_projects) == 1
    assert real_projects[0].project_slug == "proj-real"
    # Cross-project totals reflect only the real project's one session, not both projects'.
    assert "1 project(s) read and verified · 1 session(s)" in text
...
def test_discover_ledgers_reads_the_roots_own_ledger_not_only_subdirectories(tmp_path):
    """Founder-found defect, reproduced live before this test existed: `--roots
    <single-project-dir>` is the single most obvious first invocation a user tries, and
    it returned nothing at all because the original glob only matched a root's
    *subdirectories*. `<root>/.shipgate/ledger.db` (the root itself IS the project)
    must be found too, alongside `<root>/*/.shipgate/ledger.db` (the root's
    subdirectories are each a project) -- not one or the other."""
    ledger_dir = tmp_path / ".shipgate"
    ledger_dir.mkdir()
    root_db_path = ledger_dir / "ledger.db"
    _seed_healthy_ledger(root_db_path)

    sub_db_path = _make_project(tmp_path, "proj-sub")
    _seed_healthy_ledger(sub_db_path, session_id="s2")

    found = discover_ledgers([tmp_path])
    assert found == sorted([root_db_path.resolve(), sub_db_path.resolve()])

    result = analyze([tmp_path])
    slugs = {p.project_slug for p in result.projects}
    assert slugs == {tmp_path.name, "proj-sub"}
