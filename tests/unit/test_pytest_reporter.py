...
# --- Seventh founder finding: base_argv must actually be what runs, never a
# silent sys.executable default when a caller resolved something specific -----------


def test_collect_test_count_raises_for_a_nonexistent_base_argv_executable(tmp_path):
    """Proves `base_argv` is genuinely threaded through to the real subprocess call,
    not silently ignored in favor of the `sys.executable` default: pointing it at a
    path that doesn't exist must produce the real OS-level failure for THAT path, not
    a quiet success from running the default interpreter instead. Before this
    session's fix, `base_argv` didn't exist as a parameter at all -- this call would
    fail with a `TypeError` (unexpected keyword argument), a different but equally
    correct "fails for the right reason" RED."""
    nonexistent = str(tmp_path / "no" / "such" / "python.exe")
    with pytest.raises(OSError):
        collect_test_count(tmp_path, base_argv=[nonexistent])


def test_run_pytest_raises_for_a_nonexistent_base_argv_executable(tmp_path):
    """Same proof, through `run_pytest` (the one `shipgate.gate.checkers` actually
    calls)."""
    _write(tmp_path, "test_sample.py", _PASSING_TEST)
    nonexistent = str(tmp_path / "no" / "such" / "python.exe")
    with pytest.raises(OSError):
        run_pytest(tmp_path, base_argv=[nonexistent, "-m", "pytest"])


def test_run_pytest_result_command_reflects_the_provided_base_argv_not_sys_executable(tmp_path):
    """`PytestResult.command` (what `check_tests_pass` records as "actually ran") must
    be built from the real `base_argv` used, not silently `sys.executable` -- proven
    by supplying `sys.executable` explicitly through `base_argv` (rather than relying
    on the default) and confirming it's the literal first element of the real run's
    recorded command."""
    _write(tmp_path, "test_sample.py", _PASSING_TEST)
    result = run_pytest(tmp_path, base_argv=[sys.executable, "-m", "pytest"])
