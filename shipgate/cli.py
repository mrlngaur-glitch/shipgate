...
Commands land per the phase plan:
  analyze             -> founder-authorized scope addition, built this session, outside
                         the original Launch Month plan. NOT report §8.1 Week 1's
...
def analyze(
    roots: list[Path] = typer.Option(
        ...
        "--roots",
        help=(
            "A project directory itself (its own .shipgate/ledger.db is read directly) or a "
            "directory whose immediate subdirectories are each a project (scanned for "
            "*/.shipgate/ledger.db) -- both forms are checked for every value given "
