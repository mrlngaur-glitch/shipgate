...
**Founder-authorized scope addition**, not part of the original Launch Month plan --
authorized directly by the founder as a read-only, cross-project reporting layer over
ledgers this project already writes, not built silently as if it had always been in
scope.
...
class ProjectStats:
    ...
    `looks_synthetic` — see `_looks_synthetic`'s own docstring for the rule and why it
    exists (founder-found defect: a perf-testing fixture with 2000 fabricated sessions,
    all sharing one identical timestamp, rendered as an ordinary "100% green" project —
    exactly the statistics-integrity failure this product exists to prevent, one layer
    up from the ledger it's built to audit)."""
...
def discover_ledgers(roots: list[Path]) -> list[Path]:
    """`<root>/.shipgate/ledger.db` (the root IS the project) plus
    `<root>/*/.shipgate/ledger.db` (the root's immediate subdirectories are projects)
    for every root given, deduplicated (the same resolved path found under two
    different `--roots` values, or matched by both patterns, counts once) and sorted
    for deterministic output.

    **Founder-found defect, fixed:** the original version only matched subdirectories
    of a root, never the root's own ledger — so `--roots <single-project-dir>`, the
    single most obvious first invocation a user would try, returned nothing at all.
    Reproduced directly: `shipgate analyze --roots "<a real single-project checkout>"`
    rendered the vacuous-pass message even though that checkout's own
    `.shipgate/ledger.db` existed and had real, non-empty content.

    A root that doesn't exist is silently skipped at this layer — an empty glob is not
    an error here; `AnalyzeResult.is_vacuous` is what applies the vacuous-pass rule to
    the final zero-ledgers-found case, once across all roots together, not per root."""
    found: set[Path] = set()
    for raw_root in roots:
        root = Path(raw_root)
        if not root.is_dir():
            continue
        direct = root / ".shipgate" / "ledger.db"
        if direct.is_file():
...
def _looks_synthetic(session_count: int, first_activity: str | None, last_activity: str | None) -> bool:
    """A property of the data, not a directory name or path exclusion (the founder's
    own explicit constraint on the fix). Real usage across more than one session
    essentially never lands on one exactly-identical instant — genuine sessions start
    and end at different times by construction. More than one session sharing a single
    identical first/last-activity timestamp is the same shape a hand-rolled perf
    fixture produces (every row stamped with one constant time to make the fixture fast
    to generate), and is exactly what was found live: a 2000-session fixture directory
    (`scratch_perf2/`, untracked local perf-testing leftover, not part of the shipped
    repository) with `first_activity == last_activity == '2026-08-17T00:00:00Z'` for
    every one of its 2000 sessions.

    Deliberately conservative: `session_count <= 1` is never flagged, because one
    session naturally has identical first and last activity when it recorded only one
    event, or when a project is brand new — that is real, ordinary data, not a
    fixture's signature. The signature is specifically *multiple* sessions collapsing
    onto one instant."""
    return session_count > 1 and first_activity is not None and first_activity == last_activity
...
def gather_one_ledger(ledger_path: Path) -> ProjectStats | LedgerFailure:
    ...
    return ProjectStats(
        ...
        looks_synthetic=_looks_synthetic(session_count, first_activity, last_activity),
