...
def _project_section(project: ProjectStats, console: Console) -> None:
    ...
    if project.looks_synthetic:
        console.print(
            Text(
                "⚠ SYNTHETIC-LOOKING DATA: more than one session, but every one of them "
                "shares the exact same first/last-activity timestamp — real usage across "
                "sessions essentially never lands on one identical instant; this is the "
                "signature of a generated perf/test fixture, not organic use. Shown here "
                "for transparency; excluded from the cross-project totals below so it "
...
def _cross_project_section(result: AnalyzeResult, console: Console) -> None:
    ...
    synthetic_projects = [p for p in result.projects if p.looks_synthetic]
    real_projects = [p for p in result.projects if not p.looks_synthetic]
    if synthetic_projects:
        names = ", ".join(p.project_slug for p in synthetic_projects)
        console.print(
            Text(
                f"{len(synthetic_projects)} project(s) excluded from the totals below as "
                f"synthetic-looking data — not silently dropped, named here: {names} (see "
                "the ⚠ banner in each one's own section above for why).",
                style="bold yellow",
            )
        )
    if not real_projects:
        console.print(
            Text(
                "No non-synthetic-looking project ledger to aggregate — see the exclusion "
                "line above.",
                style="bold yellow",
            )
        )
    ...
    healthy = [p for p in real_projects if p.session_count > 0]
    if not healthy:
        console.print(
            Text(
                f"{len(real_projects)} ledger(s) read and verified, all with 0 sessions "
    ...
    total_sessions = sum(p.session_count for p in real_projects)
    total_claims = sum(p.claim_count for p in real_projects)
    total_gate = sum(p.gate_evaluations_total for p in real_projects)
    total_gate_green = sum(p.gate_evaluations_green for p in real_projects)
    total_gate_not_green = total_gate - total_gate_green

    console.print(
        Text(
            f"{len(real_projects)} project(s) read and verified · {total_sessions} session(s) · "
            f"{total_claims} claim(s)"
        )
    )
    console.print(
        _cross_project_token_line(
            aggregate_token_metric(real_projects, "tokens_input"),
            aggregate_token_metric(real_projects, "tokens_output"),
            aggregate_token_metric(real_projects, "tokens_cache_read"),
        )
    )
    if total_gate:
    ...
    console.print(_verdicts_table("Verdicts by class, across all projects", aggregate_verdicts_by_class(real_projects)))
    console.print(
        _reasons_table(
            "Most common failure reasons, across all projects", aggregate_failure_reasons(real_projects)
        )
    )
